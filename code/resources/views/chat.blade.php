...
$(function(){
	...
	$.ajax({
			...
			success : function(data){
				...
				myheadimg = '/avatar?uri=' + escape(res.User.HeadImgUrl);
				//登陆用户信息
				var ustr = '<header>'
				...
				for (var key in userlist) {
					var img = '/avatar?uri=' + escape(userlist[key].HeadImgUrl);
				...
				if(res.BaseResponse.Ret != 0){
					...
					for (var key in messagelist) {
						...
						if (messagelist[key].StatusNotifyCode == 0){
							...
							var userHeadimg = '/avatar?uri=' + escape(uri);
							str += '<li>'
		                	...
		                	//机器人自动回复，不需要注释掉即可
		                	$.post('/tuling',{content:messagelist[key].Content,toUsername:messagelist[key].FromUserName},function(data){
